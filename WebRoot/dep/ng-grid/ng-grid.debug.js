...
var COL_CLICK_FN = /COL_CLICK_FN/g;
var COL_CLICK_PARAMS = /COL_CLICK_PARAMS/g;
...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.cellType = (colDef.cellType === undefined) ? '' : colDef.cellType;
    self.linkFn = (colDef.linkFn === undefined) ? '' : colDef.linkFn;
    self.linkParams = (colDef.linkParams === undefined) ? '' : colDef.linkParams;
    
    self.checkboxDisabled= (colDef.checkboxDisabled === undefined) ? false : colDef.checkboxDisabled;
    ...
    self.cellImgTemplate = $templateCache.get('cellImgTemplate.html');
    self.cellLinkTemplate = $templateCache.get('cellLinkTemplate.html').replace(CUSTOM_FILTERS, self.cellFilter ? "|" + self.cellFilter : "");
    
    self.cellCheckboxTemplate = $templateCache.get('cellCheckboxTemplate.html').replace('disabled', self.checkboxDisabled?'disabled':"");
    ...
    self.setVars = function (fromCol) {
        ...
        self.checkboxDisabled = fromCol.checkboxDisabled;
        
        self.cellType = fromCol.cellType;
        self.linkFn = fromCol.linkFn;
        self.linkParams = fromCol.linkParams;
...
var ngFooter = function ($scope, grid) {
    ...
    $scope.pageForward = function() {
        var page = new Number($scope.pagingOptions.currentPage);
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    var defaults = {
        ...
        */
        ...
        // 原本是55，后来因为删掉了selectedItems.length少掉了一行，于是改为35
        footerRowHeight: 35,
    ...
    self.getTemplate = function (key) {
        ...
        if (t && !TEMPLATE_REGEXP.test(t)) {
        	...
        	var dKey = key + ".html";
        	var html= $templateCache.get(dKey);
        	if(key=='rowTemplate' && self.config["onDblClick"] != '')
        	{
        		html=html.replace(/doNgGridDblClick/, self.config["onDblClick"]);
        	}
        	$templateCache.put(uKey, html);
//            $templateCache.put(uKey, $templateCache.get(dKey));
    ...
    $scope.showNgViewport = self.config.showNgViewport;
        ...
        compile: function() {
            return {
                pre: function($scope, iElement) {
                    ...
                    function getLinkFnParams (params) {
                	var  linkFnParams = [];
                        if (params.length > 0) {
                            var tempParams = params.split(',');
                            for(var i = 0; i < tempParams.length; i++) {
                                linkFnParams.push(utilityService.preEval('row.entity.' + tempParams[i]));
                            }
                        }
                        return linkFnParams;
                    }
                    if ($scope.col.cellType == "img") {
                        html = $scope.col.cellImgTemplate.replace(COL_FIELD, utilityService.preEval('row.entity.' + $scope.col.field) );
                    }
                    else if ($scope.col.cellType == 'link') {
                        html = $scope.col.cellLinkTemplate;
                        html = html.replace(COL_FIELD, 'row.entity.' + $scope.col.field);
                        html = html.replace(COL_CLICK_FN, $scope.col.linkFn);
                        html = html.replace(COL_CLICK_PARAMS, getLinkFnParams($scope.col.linkParams));
                    } 
                    else if($scope.col.cellType == 'checkbox'){
                    	html = $scope.col.cellCheckboxTemplate;
                    }
                    else if ($scope.col.enableCellEdit) {
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('cellEditTemplate.html',
    ...
    $templateCache.put('cellImgTemplate.html',
        "<div class=\"ngCellText\" ng-class=\"col.colIndex()\" align=\"center\"><img ng-src=\"{{COL_FIELD}}\" style=\"height: 100%;\" show-big-img/></div>"
    );


    $templateCache.put('cellLinkTemplate.html',
        "<div class=\"ngCellText\" ng-class=\"col.colIndex()\" align=\"center\"><a ng-click=\"COL_CLICK_FN(COL_CLICK_PARAMS)\" ><span ng-cell-text style=\"text-decoration: underline;\">{{COL_FIELD CUSTOM_FILTERS}}</span></a></div>"
    );
    
    $templateCache.put('cellCheckboxTemplate.html',
    		"<input type ='checkbox' disabled ng-checked='{{row.getProperty(col.field)}}' />"
    );
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "        </div>\r" +
    "\n" +
    /* 这个selectedItems.length有个bug的，反正没什么用，删了吧 */
    ...
    "\n" +
    "            <span style=\"float: left; font-size: 12px;line-height: 20px; \" class=\"ngLabel\">{{i18n.ngPageSizeLabel}}</span>\r" +
    "\n" +
    "            <select style=\"float: left; height: 20px; width: 50px;\" ng-model=\"pagingOptions.pageSize\" >\r" +
    "\n" +
    ...
    "\n" +
    "            <input class=\"ngPagerCurrent\" min=\"1\" max=\"{{currentMaxPages}}\" type=\"text\" style=\"width:50px; height: 20px; margin-top: 1px; line-height: 20px; padding: 0 4px; box-sizing: border-box; -moz-box-sizing: border-box; -webkit-box-sizing: border-box;\" ng-model=\"pagingOptions.currentPage\"/>\r" +
    "\n" +
  ...
  $templateCache.put('gridTemplate.html',
    ...
    "</div>\r" +
    "\n" +
    "<div  unselectable=\"on\" ng-viewport ng-class=\"{'ui-widget-content': jqueryUITheme, 'ngViewport': showNgViewport}\" ng-style=\"viewportStyle()\">\r" +
    "\n" +
  ...
  $templateCache.put('headerCellTemplate.html',
    ...
    "\n" +
    "    <div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText text-left\"><span translate=\"{{col.displayName}}\"></span></div>\r" +
    "\n" +
  ...
  $templateCache.put('rowTemplate.html',
    "<div ng-style=\"{ 'cursor': row.cursor }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" ng-dblclick=\"doNgGridDblClick(row, this.parent)\" class=\"ngCell {{col.cellClass}}\">\r" +
    "\n" +
